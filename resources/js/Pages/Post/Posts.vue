<template>
    <div>
        ...
        <Dropdown align="left">
            <template #trigger>
                <button type="button" v-if="$page.props.auth.user">{{ $page.props.auth.user.name }}</button>
                <div v-else>{{ redirectToHomeIfNotLoggedIn() }}</div>
            </template>
            <template #content>
                <DropdownLink :href="'/logout'" method="post">로그아웃</DropdownLink>
            </template>
        </Dropdown>
        <table>
            <colgroup>
        ...
        <form class="search1" @submit.prevent="search(inputValue)">
            ...
            <button class="button1" type="submit">게시글 검색</button>
        </form>
    </div>
</template>
